...
function queueSimulation() {
  for (var i = 0; i < numbers.length; i++) {
    ...
    //var list_text = document.createTextNode(number.toString());
    //list_item.appendChild(list_text);
    //将数字转化为高度
    list_item.style.height = number*5+"px";
...
function firstIn() {
  ...
  if(!checkCondition(input_number)){
    return false;
  }else {
    clearQueue();
    numbers.unshift(parseInt(input_number));
    queueSimulation();
  }
}
//右侧入
function lastIn(number) {
  ...
  if (!checkCondition(input_number)) {
    ...
    clearQueue();
    numbers.push(parseInt(input_number));
    queueSimulation();
  }
}
//左侧出
...
//检查输入是否在规定范围内以及数组长度是否超过60
function checkCondition(input_number) {
  if (parseInt(input_number)<10||parseInt(input_number)>100) {
    alert("当前输入值不在范围内，请输入10到100之间的数字");
    return false;
  }
  if (input_number==="") {
    alert("当前输入值为空");
  ...
  if (numbers.length === 60) {
    alert("当前队列元素已达到最大值60");
    return false;
  }
  return true;
}
//升序重排
function asceNumbers(number1,number2) {
  clearQueue();
  numbers.sort(asceSort);
  queueSimulation();
}
function asceSort(number1,number2) {
  return number1 - number2;
}
//降序重排
function descNumbers(number1,number2) {
  clearQueue();
  numbers.sort(descSort);
  queueSimulation();
}
function descSort(number1,number2) {
...
function btnInit() {
  ...
  var asceBtn = document.getElementById("resort_asce");
  asceBtn.onclick = asceNumbers;
  var descBtn = document.getElementById("resort_desc");
  descBtn.onclick = descNumbers;
}
//
function init() {
  queueSimulation();
